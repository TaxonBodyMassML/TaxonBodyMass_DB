check_enriched <- function(dat) {
  ...
  for (col in rank_cols) {
    ...
    if (n_na > 0)
      warn <- c(warn, sprintf("## Missing `%s` after all enrichment stages (%d rows)\n\n%s",
  ...
  bad_fmt <- dat[!is.na(dat$species) & !grepl("^[A-Z][a-z]+ [a-z]+$", dat$species), ]
  if (nrow(bad_fmt) > 0)
    warn <- c(warn, sprintf("## Species names not in 'Genus epithet' format (%d rows)\n\n%s",
  ...
  if (nrow(mismatch) > 0)
    warn <- c(warn, sprintf("## genus column does not match species prefix (%d rows)\n\n%s",
  ...
  if ("gbif_confidence" %in% names(dat)) {
    ...
    if (nrow(low_conf) > 0)
      warn <- c(warn, sprintf("## Low GBIF confidence (75-89) (%d rows)\n\n%s",
  ...
  if ("gbif_status" %in% names(dat)) {
    ...
    if (nrow(doubtful) > 0)
      warn <- c(warn, sprintf("## GBIF status DOUBTFUL (%d rows)\n\n%s",
  ...
  if ("species_changed" %in% names(dat)) {
    ...
    if (nrow(changed) > 0)
      warn <- c(warn, sprintf("## Species name changed during enrichment (%d rows)\n\n%s",
  ...
  if ("log10_range" %in% names(dat)) {
    ...
    if (nrow(moderate_range) > 0)
      warn <- c(warn, sprintf("## Moderate mass disagreement (log10 range 1-2) (%d species)\n\n%s",
  ...
  if (length(errs) > 0) {
    writeLines(c(sprintf("# TaxonBodyMass_DB Errors -- %s\n", now), errs),
      file.path(wd_root, "reports", "errors.md"))
    warning(sprintf("%d error type(s) found -- see reports/errors.md", length(errs)),
      immediate. = TRUE)
  } else {
    writeLines(c(sprintf("# TaxonBodyMass_DB Errors -- %s\n", now),
      "No errors found."),
  ...
  if (length(warn) > 0) {
    writeLines(c(sprintf("# TaxonBodyMass_DB Warnings -- %s\n", now), warn),
      file.path(wd_root, "reports", "warnings.md"))
    message(sprintf("%d warning type(s) -- see reports/warnings.md", length(warn)))
  } else {
    writeLines(c(sprintf("# TaxonBodyMass_DB Warnings -- %s\n", now),
      "No warnings."),
      file.path(wd_root, "reports", "warnings.md"))
